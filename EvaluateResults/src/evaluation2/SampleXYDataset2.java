...
//                this.xValues[series][item] = new Double(x);
                this.xValues[series][item] = xvalues[series][item];
                if (xvalues[series][item] < minX) {
                    minX = xvalues[series][item];
                }
                if (xvalues[series][item] > maxX) {
                    maxX = xvalues[series][item];
                }

//                double y = (Math.random() + 0.5) * 6 * x + x;
//                this.yValues[series][item] = new Double(y);
                this.yValues[series][item] = yvalues[series][item];
                if (yvalues[series][item] < minY) {
                    minY = yvalues[series][item];
                }
                if (yvalues[series][item] > maxY) {
                    maxY = yvalues[series][item];
                }
