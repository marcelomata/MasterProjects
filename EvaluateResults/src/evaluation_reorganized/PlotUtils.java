		...
		for (String patient : keysPrototype) {
			if(patient.toLowerCase().contains("luiza")) {
				...
				if(rightReportPrototypeData.size() > 1 && rightReportHumphreyData.size() > 1 && !patient.equalsIgnoreCase("Dennis")) {
					...
					plot(false, LoadManually.intensitiesMathiasRight1, rightReportHumphreyData.get(0).getNumericIntensities(), means, meansHumphrey, false, new String[] {"Protótipo", "Humphrey", "Média no protótipo", "Média no Humphrey"});
					plot(false, LoadManually.intensitiesMathiasRight2, rightReportHumphreyData.get(1).getNumericIntensities(), means, meansHumphrey, false, new String[] {"Protótipo", "Humphrey", "Média no protótipo", "Média no Humphrey"});
				}
				return;
			}
		}
