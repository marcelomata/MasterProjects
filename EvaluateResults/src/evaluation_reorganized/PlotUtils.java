		...
		for (String patient : keysPrototype) {
			if(patient.toLowerCase().contains("luiza")) {
				...
				if(leftReportPrototypeData.size() > 1 && leftReportHumphreyData.size() > 1 && !patient.equalsIgnoreCase("Dennis")) {
					...
					plot(true, leftReportPrototypeData.get(0).getParameters().getIntensitiesAsDouble(), leftReportHumphreyData.get(0).getNumericIntensities(), means, meansHumphrey, true, new String[] {"Prot贸tipo", "Humphrey", "M茅dia no prot贸tipo", "M茅dia no Humphrey"});
					plot(true, leftReportPrototypeData.get(1).getParameters().getIntensitiesAsDouble(), leftReportHumphreyData.get(1).getNumericIntensities(), means, meansHumphrey, true, new String[] {"Prot贸tipo", "Humphrey", "M茅dia no prot贸tipo", "M茅dia no Humphrey"});
				}
...
//		plot.plot();
	...
	public static void plotByFieldIndex(Map<String, double[][][][]> squareDiffs, int index) {
		...
		Utils.setNullAsZero(xDataTotal1);
		Utils.setNullAsZero(yDataTotal1);
